...
var WikEdDiff = function () {
	...
	this.config = {
		...
		'stylesheet':
			...
			// Newline
			'.wikEdDiffNewline:before { content: "Â¶"; color: transparent; } ' +
			...
			'.wikEdDiffTabSymbol:before { content: "â†’"; font-size: smaller; color: #ccc; } ' +
			...
			'.wikEdDiffSpaceSymbol:before { content: "Â·"; color: transparent; } ' +
    ...
    'wiked-diff-ins':   '+',
    'wiked-diff-del':   '-',
    'wiked-diff-block-left':  'â—€',
    'wiked-diff-block-right': 'â–¶',
	...
	this.debugShortenText = function ( text, max, end ) {
		...
		if ( text.length > max ) {
			text = text.substr( 0, max - 1 - end ) + 'â€¦' + text.substr( text.length - end );
		}
		return '"' + text + '"';
	};
